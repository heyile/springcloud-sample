...
华为云部署spring cloud物理多组 eureka 和 config server示例项目,该项目主要是讲述如何在华为云 ServiceStage 平台上部署 spring cloud eureka 和 config server,实现高可用.
...
* eureka 参数配置
 ...
 >   1. 注册实例数 `500 以下` , CPU 申请 `0.6` 核, 限制 `1.2` 核 . 内存申请 `1.0` G 限制 `1.3` G 
 >   2. 注册实例数 `500 ~ 1000` , CPU 申请 `1.0` 核, 限制 `1.8` 核 . 内存申请 `1.2` G 限制 `2.2` G
 >   3.  **ps: 实际本地测试注册 500 实例 ,并且维持心跳. 使用大约 1G 内存**
 > 
 > * 部署config server 高可用集群, 建议实例数量为 `2`
 >   1. CPU 申请 `0.5` 核, 限制 `0.8` 核 . 内存申请 `1.2` G 限制 `1.5` G
 > 
